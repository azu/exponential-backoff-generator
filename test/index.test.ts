import { generateBackoff } from "../src";
...
describe("generateBackoff", function() {
    ...
    it("generate duration with custom factor ", () => {
        ...
        generated.forEach((duration, index) => {
            assert.ok(defaultGenerated[index] - defaultGenerated[index] * 0.5 <= duration && duration <= defaultGenerated[index] + defaultGenerated[index] * 0.5, `{${index}} ${duration}`);
        });
    });
